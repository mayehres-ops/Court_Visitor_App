...
BASE = Path(_env("GUARDIAN_BASE", r"C:\GoogleSync\GuardianShip_Easy_App"))
LOCAL_PDF_INPUT_DIR = _env("PDF_INPUT_DIR", str(Path(r"C:\GoogleSync\GuardianShip_Easy_App\New Files")))
...
ARP_OCR_MODE = "path"           # "path" / "bytes" / "mixed"
...
def parse_arp_fields(text: str) -> dict | None:
    ...
    # First try the new name line parser
    name_line = None
    for line in gslice:
        ...
        if (re.search(r'name(?:\(s\))?s?\s*(?:[:\-]|\s)', line, re.I) or
            ...
            re.search(r'2\.\s*guardian\(s\)\s*:\s*name\(s\)', line, re.I)):
            name_line = line
            break
    ...
    # Fallback to old method if name line parser didn't work
    ...
    if g1 and not g2:
        ...
        # Also look for "and [Name]" patterns in the guardian slice
        if not g2:
            for line in gslice:
                # Look for "and [First Last]" pattern, but exclude common non-name words
                if re.search(r'\band\s+[A-Z][a-z]+\s+[A-Z][a-z]+', line, re.I):
                    match = re.search(r'\band\s+([A-Z][a-z]+\s+[A-Z][a-z]+)', line, re.I)
                    if match:
                        candidate = match.group(1)
                        # Exclude common non-name words that might appear after "and"
...
def _looks_like_human_name(s: str) -> bool:
    ...
    # disallow role-y tokens as a "name"
...
def _split_guardian_field_by_separators(value: str) -> tuple[str, str]:
    ...
    separators = [
        ...
        r'\s{3,}',            # 3+ spaces (but not 2 spaces which might be normal spacing)
    ]
    ...
    for f in files:
        ...
        try:
            ...
            if "arp" in lower_name:
                ...
                if len(text or "") < 80:
                    log("  Little text found; using ARP OCR (page 1)...")
                    t4 = extract_text_with_ocr_for_arp(pdf_bytes, psm=4)
                    t6 = extract_text_with_ocr_for_arp(pdf_bytes, psm=6)
