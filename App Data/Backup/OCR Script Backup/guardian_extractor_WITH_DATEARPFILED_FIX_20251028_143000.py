...
def extract_text_with_document_ai(pdf_bytes: bytes) -> str:
    ...
    if not project_id or not processor_id:
        log("⚠️ Document AI not configured (missing PROJECT_ID or PROCESSOR_ID); skipping.")
    ...
    if not os.path.exists(VISION_CREDENTIALS_FILE):
        log(f"⚠️ Document AI credentials missing at {VISION_CREDENTIALS_FILE}; skipping.")
        extract_text_with_document_ai._hard_disabled = True
        return ""
    ...
    try:
        ...
        log(f"⚠️ Document AI library not installed: {e}")
    ...
    try:
        ...
        log(f"⚠️ Document AI request failed: {e}")
        return ""
...
def extract_arp_filed_date(t: str) -> str:
    ...
    Tries to recognize clerk stamps such as:
      ...
      'Filed: Jul 22, 2025'
      'Filed 07/22/2025'
      'FEED FOR RECORD 20 MAR 28' (OCR misread, YY MON DD format)
    ...
    # Priority 1: 'YY MON DD' format near file stamp (common in handwritten stamps)
    # Example: "FEED FOR RECORD\n20 MAR 28 P 3 41" -> March 28, 2020
    m = re.search(
    ...
    # Priority 2: Numeric near 'Filed/Entered' (including 'Filed for Record')
    ...
    # Priority 3: 'Month 22, 2025' near 'Filed/Entered' (including 'Filed for Record')
    m = re.search(
    ...
    # Priority 4: 'YYYY Mon 22' near 'Filed/Entered' (including 'Filed for Record')
    m = re.search(
    ...
    # Priority 5: 'Mon 22 2025' near 'Filed/Entered' (including 'Filed for Record')
    m = re.search(
        rf'({filed_pattern})\b.{{0,60}}?'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
    ...
    # Last resort: anywhere 'YYYY Mon DD' (but NOT if preceded by "Updated")
    # This catches file stamps that were misread entirely
    m = re.search(
        r'(?<!Updated\s.{0,20})(\d{4})\s+'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
        r'\s+(\d{1,2})(?:st|nd|rd|th)?',
        t, re.I
                ...
                try:
                    ...
                    if row_hint and row_hint.get("causeno"):
                        ...
                        if row_hint.get("wardfirst") and row_hint.get("wardlast"):
                            ORDER_WARD_NAMES[row_hint["causeno"]] = {
                                'first': row_hint.get("wardfirst", ""),
                                'middle': row_hint.get("wardmiddle", ""),
                                'last': row_hint.get("wardlast", "")
                            }
                            log(f"  Stored ORDER ward name for {row_hint['causeno']}: {row_hint.get('wardfirst')} {row_hint.get('wardmiddle')} {row_hint.get('wardlast')}")
                except Exception:
                    pass
        if ORDER_CAUSES:
