...
# --- BEGIN: safe console I/O wrapper for Windows ---
import sys
...
# Excel workbook location(s)
# - Use the test file while tuning, then switch to prod when ready.
# When ready for production, switch to:
# LOCAL_EXCEL_PATH = r"C:\GoogleSync\Guardianship Files\data files\ward_guardian_info.xlsx"
LOCAL_EXCEL_PATH = _env("EXCEL_PATH", str(BASE / "App Data" / "ward_guardian_info.xlsx"))
# Google Vision service-account key (JSON)
...
# Document AI config (optional - loads from env file if exists)
# This provides better handwriting recognition (90-95% vs 85-90% with Vision API)
_DOCAI_CONFIG_FILE = r"C:\configlocal\API\document_ai_config.env"
if os.path.exists(_DOCAI_CONFIG_FILE):
    try:
        with open(_DOCAI_CONFIG_FILE, 'r') as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith('#') and '=' in line:
                    key, value = line.split('=', 1)
                    key, value = key.strip(), value.strip()
                    if value and key:  # Only set if value is not empty
                        os.environ[key] = value
        print(f"Loaded Document AI config from {_DOCAI_CONFIG_FILE}")
    except Exception as e:
        print(f"Could not load Document AI config: {e}")
...
def extract_text_with_ocr_for_arp_multipage(pdf_path: str, max_pages: int = 2):
    """
    Extract text from first N pages of ARP using OCR.
    Optimized for ARP documents where relevant data is typically on first 2 pages.
    """
    all_text = []
    
    for page_idx in range(max_pages):
        try:
            log(f"  OCR processing page {page_idx + 1} of {max_pages}")
            page_text = extract_text_with_ocr_for_arp(pdf_path, page_index=page_idx)
            if page_text and len(page_text.strip()) > 50:  # Only add if substantial content
                all_text.append(page_text)
                log(f"  Page {page_idx + 1}: extracted {len(page_text)} characters")
            else:
                log(f"  Page {page_idx + 1}: insufficient content, skipping")
        except Exception as e:
            log(f"  OCR failed for page {page_idx + 1}: {e}")
            break
    
    combined_text = "\n\n".join(all_text)
    log(f"  Multi-page OCR: {len(all_text)} pages processed, {len(combined_text)} total characters")
...
def capture_arp_address_by_labels(text: str,
                                  ...
                                  max_lines: int = 4) -> str:
    """
    Enhanced address capture from an ARP section by looking for both the "Address" label
    ...
    # --- ENHANCED COMBINE LOGIC WITH VALIDATION ---
    if raw_street and raw_city:
        combined = f"{raw_street}, {raw_city}"
        cleaned = clean_address(combined)
        # Validate that we have a complete address
        if _is_complete_address(cleaned):
            return cleaned
        else:
            # Try to find better city/state/zip in nearby lines
            for i, ln in enumerate(lines):
                if ln.strip() == raw_street:
                    for j in range(max(0, i-2), min(len(lines), i+3)):
                        if j != i and lines[j].strip():
                            test_line = lines[j].strip()
                            if ENHANCED_CITY_STATE_ZIP_RE.search(test_line) or re.search(r",[ ]*[A-Za-z]{2,}[ ]+\d{5}(?:-\d{4})?$", test_line):
                                better_combined = f"{raw_street}, {test_line}"
                                better_cleaned = clean_address(better_combined)
                                if _is_complete_address(better_cleaned):
                                    return better_cleaned
            return cleaned  # Return what we have even if incomplete
    elif raw_street:
        # If we only have street, try to find city/state/zip in nearby lines
        for i, ln in enumerate(lines):
            if ln.strip() == raw_street:
                for j in range(max(0, i-2), min(len(lines), i+3)):
                    if j != i and lines[j].strip():
                        test_line = lines[j].strip()
                        if ENHANCED_CITY_STATE_ZIP_RE.search(test_line) or re.search(r",[ ]*[A-Za-z]{2,}[ ]+\d{5}(?:-\d{4})?$", test_line):
                            combined = f"{raw_street}, {test_line}"
                            cleaned = clean_address(combined)
                            if _is_complete_address(cleaned):
                                return cleaned
                # Enhanced: try to stitch multiple lines if single line doesn't work
                for j in range(max(0, i-1), min(len(lines), i+4)):
                    if j != i and lines[j].strip():
                        test_line = lines[j].strip()
                        # Look for partial city/state/zip patterns
                        if re.search(r'\b[A-Za-z]{2,}\b', test_line) and re.search(r'\b\d{5}(?:-\d{4})?\b', test_line):
                            combined = f"{raw_street}, {test_line}"
                            cleaned = clean_address(combined)
                            if _is_complete_address(cleaned):
                                return cleaned
        return clean_address(raw_street)
    elif raw_city:
        return clean_address(raw_city)
    else:
        return ""
...
# Enhanced address patterns for better OCR error handling
ENHANCED_STREET_LINE_RE = re.compile(
    r"""
    ^\s*
    \d{1,5}                              # Street number
    [\s,.-]*
    [A-Za-z0-9'.\- ]+                    # Street name (more flexible)
    \s+
    (?:St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|
       Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|
       Trl|Trail|Ter|Terrace|Way|Loop|Pass|Cove|Circle|Cir|Hwy|Highway)\.?
    (?:\s+[A-Za-z0-9#.\- ]+)?            # Optional unit
    \s*$
    """,
    re.IGNORECASE | re.VERBOSE
)

# Enhanced city/state/zip pattern - more flexible
ENHANCED_CITY_STATE_ZIP_RE = re.compile(
    r"""
    ^\s*
    (?:City/State/Zip\s*:)?\s*           # Optional label prefix
    [A-Za-z][A-Za-z .'\-]+               # City (one or more words)
...
def extract_text_with_document_ai(pdf_bytes: bytes) -> str:
    """
    Google Document AI OCR - BEST for handwriting (90-95% accuracy).
    Replaces Vision API for handwritten ARP processing.

    Requires:
    - pip install google-cloud-documentai
    - DOCUMENT_AI_PROJECT_ID, DOCUMENT_AI_PROCESSOR_ID, DOCUMENT_AI_LOCATION env vars
    - Same service account JSON as Vision API (VISION_CREDENTIALS_FILE)
    """
    # If we already failed init once this run, don't try again
    if getattr(extract_text_with_document_ai, "_hard_disabled", False):
        return ""
    ...
    if not project_id or not processor_id:
        log("[WARN]️ Document AI not configured (missing PROJECT_ID or PROCESSOR_ID); skipping.")
        extract_text_with_document_ai._hard_disabled = True
        return ""

    # No creds: disable for this run
    if not os.path.exists(VISION_CREDENTIALS_FILE):
        log(f"[WARN]️ Document AI credentials missing at {VISION_CREDENTIALS_FILE}; skipping.")
        extract_text_with_document_ai._hard_disabled = True
        return ""
    ...
    try:
        ...
        log(f"[WARN]️ Document AI library not installed: {e}")
        log("   Run: pip install google-cloud-documentai")
        extract_text_with_document_ai._hard_disabled = True
        return ""
    ...
    try:
        ...
        return normalize_unicode_noise(clean_text(text))

    except Exception as e:
        log(f"[WARN]️ Document AI request failed: {e}")
        return ""
...
def extract_text_with_vision(pdf_bytes: bytes) -> str:
    ...
    NOTE: Document AI is preferred for handwriting (90-95% vs 85-90%).
    """
    # If we already failed init once this run, don't try again
    ...
    # No creds: disable for this run
    if not os.path.exists(VISION_CREDENTIALS_FILE):
        log(f"[WARN]️ Vision credentials missing at {VISION_CREDENTIALS_FILE}; skipping Vision fallback.")
    ...
    try:
        ...
        log(f"[WARN]️ Could not init Vision client: {e}")
        extract_text_with_vision._hard_disabled = True
        return ""
    ...
    try:
        ...
        for img in images:
            ...
            if getattr(resp, "error", None) and getattr(resp.error, "message", ""):
                log(f"[WARN]️ Vision API error: {resp.error.message}")
                continue
        ...
        log(f"[WARN]️ Vision request failed: {e}")
        return ""
...
def extract_ward_name_candidates_from_order(t: str) -> list[str]:
    ...
    # NEW PATTERN: Handle cases where ward name appears on a line by itself between headers
    # e.g., "IN THE GUARDIANSHIP OF � IN THE PROBATE COURT #2\n�\nEMMA TONEY � OF"
    # Look for lines that start with capital letters and contain 2-4 words (typical human names)
    # This must come after "GUARDIANSHIP OF" and before "INCAPACITATED PERSON" or similar text
    lines = T.splitlines()
    found_guardianship = False
    for i, line in enumerate(lines):
        if re.search(r'IN\s+THE\s+GUARDIANSHIP\s+OF', line, re.I):
            found_guardianship = True
            print(f"  DEBUG standalone: Found GUARDIANSHIP line at {i}")
            continue
        if found_guardianship and i < len(lines):
            # Look at the next few lines for a potential ward name
            # Ward name should be: capitalized words, 2-4 words total, not contain common header text
            line_clean = re.sub(r'[^\w\s\'-]', ' ', line).strip()  # Remove special chars but keep spaces
            line_clean = re.sub(r'\s+', ' ', line_clean)  # Normalize spaces
            # Remove common trailing words like "OF" that appear after the name
            line_clean_before = line_clean
            line_clean = re.sub(r'\s+OF\s*$', '', line_clean, flags=re.I)
            line_clean = line_clean.strip()
            words = line_clean.split()
            print(f"  DEBUG standalone line {i}: orig={line!r}, clean_before={line_clean_before!r}, clean_after={line_clean!r}, words={words}")
            # Check if this looks like a name: 2-4 words, each word starts with capital, reasonable length
            if (2 <= len(words) <= 4 and
                all(w[0].isupper() and len(w) >= 2 for w in words if w) and
                len(line_clean) >= 5 and
                not looks_like_noise(line_clean) and
                _looks_like_human_name(line_clean)):
                cands.append(line_clean)
                print(f"  Found ward name from standalone line pattern: {line_clean!r}")
                found_guardianship = False  # Reset so we don't keep matching random lines
                break
            # If we've gone past a few lines without finding a name, stop looking
            if i > 5:  # Don't search more than 5 lines ahead
                found_guardianship = False
                print(f"  DEBUG standalone: Stopping search at line {i} (i > 5)")
    ...
    order_patterns = [
        # Look for ward name followed by "OF" then "AN INCAPACITATED PERSON" (common in ORDER documents)
        # This pattern handles cases where the ward name is on its own line between "GUARDIANSHIP OF" and "AN INCAPACITATED PERSON"
        # Allow both Title Case and ALL CAPS names
        # This must come FIRST because it's more specific than the generic patterns below
        r'IN\s+THE\s+GUARDIANSHIP\s+OF.*?[\n\r].*?[\n\r]\s*([A-Z][A-Za-z]*(?:\s+[A-Z][A-Za-z]*)+).*?OF.*?[\n\r].*?AN\s+INCAPACITATED\s+PERSON',
    ...
    for idx, pattern in enumerate(order_patterns):
        m = re.search(pattern, T, re.I | re.M | re.DOTALL)
        if m:
            ward_name = m.group(1).strip()
            raw_name = ward_name
            ...
            passes_checks = ward_name and len(ward_name) >= 3 and not looks_like_noise(ward_name) and _looks_like_human_name(ward_name)
            print(f"  DEBUG ORDER pattern {idx}: raw={raw_name!r}, clean={ward_name!r}, len={len(ward_name)}, noise={looks_like_noise(ward_name)}, human={_looks_like_human_name(ward_name)}, passes={passes_checks}")
            if passes_checks:
                cands.append(ward_name)
...
def extract_ward_name_candidates_from_arp(t: str) -> list[str]:
    ...
    # The WARD block appears BELOW the file stamp area, making it more reliable
    ward_block_patterns = [
        # Pattern: Look for "1. WARD:" section header followed by "Name [Name] Age" or "Name [Name] DOB"
        # MUST start with "1." to avoid matching "Relationship to Ward:" and similar phrases
        r'1\.\s*WARD\s*:?\s*Name\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3})\s+Age',
        r'1\.\s*WARD\s*:?\s*Name\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3})\s+\d+\s*/\s*DOB',
    ]
    ...
    for pattern in ward_block_patterns:
        ...
        if m:
            ...
            if ward_name and len(ward_name) >= 3 and _looks_like_human_name(ward_name):
                ...
                # WARD block is most reliable - if we find it here, return immediately
                # This avoids file stamp contamination from top patterns
                c2 = _strip_qualifiers(ward_name)
                if _looks_like_human_name(c2):
                    return [c2]
    ...
    for i, line in enumerate(lines[:10]):  # Check first 10 lines
        ...
        # But exclude common non-ward names AND file stamps
        if (re.search(r'^[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}$', line_clean) and
            ...
            not re.search(r'(?:Filed|No\.|C-1-PB-|Cause\s+No)', line_clean, re.I) and  # Exclude file stamps
            not re.search(r'(?:Updated|Filed\s*for\s*record|LED\s*FOR\s*RECORD)', line_clean, re.I) and  # Exclude file stamp keywords
            not re.search(r'(?:Limon-Mercado|Mercado|Dyana|Jaimes)', line_clean, re.I) and  # Exclude specific file stamp names
...
def choose_best_ward_name(cands: list[str]) -> tuple[str, str, str]:
    ...
    Uses Claude Code's recommendations: source priority weighting, frequency-based selection,
    and aggressive file stamp filtering.
    """
    if not cands:
        return ("", "", "")
    
    # DEBUG: Log the candidate analysis
    print(f"DEBUG: Raw candidates = {cands}")
    
    # Claude Code's Source Priority Weighting
    SOURCE_WEIGHTS = {
        'ARP top caps same line': 10,  # Highest - specific pattern match
        'ARP top same line': 9,
        'ARP WARD block': 5,
        'ARP top lines': 3  # Lowest - most prone to errors
    }
    
    # Filter out known file stamp names and patterns (more contextual)
    file_stamp_names = {
        "dyana", "limon-mercado", "mercado", "jaimes", "travis", "county", "clerk"
        # Removed "blair", "hicks" as they might be legitimate names
    }
    
    # Remove file stamp contaminated candidates
    filtered_cands = []
    for cand in cands:
        cand_lower = cand.lower()
        # Skip if it's a known file stamp name
        if any(name in cand_lower for name in file_stamp_names):
            continue
        # Skip if it contains file stamp keywords
        if re.search(r'\b(updated|filed|clerk|county|travis)\b', cand_lower):
            continue
        # Skip if it looks like a file stamp pattern (very short or contains special chars)
        if len(cand.split()) < 2 or re.search(r'[^\w\s\'-]', cand):
            continue
        # Skip names with repeated words (like "Cos Cos Cas")
        words = cand.split()
        if len(set(words)) < len(words):  # Has duplicate words
            continue
        filtered_cands.append(cand)
    
    # Use filtered candidates if any, otherwise fall back to original
    pool = filtered_cands if filtered_cands else cands
    print(f"DEBUG: Filtered candidates = {filtered_cands}")
    print(f"DEBUG: Final pool = {pool}")
    
    # Claude Code's Frequency-Based Selection
    name_frequency = {}
    for candidate in pool:
        name_frequency[candidate] = name_frequency.get(candidate, 0) + 1
    
    # Prefer names that appear multiple times
    def score(s: str):
        tokens = len(s.split())
        frequency_boost = name_frequency.get(s, 0) * 5  # Boost for frequency
        return (abs(tokens - 3), -frequency_boost, tokens, len(s))
    
    pool.sort(key=score)
    best = pool[0]
    
    # Additional filtering: avoid "estate" references
...
def to_slashes(date_str: str) -> str:
    ...
    # Clean up common OCR artifacts and separators
    cleaned = date_str.strip()
    # Remove leading colons and spaces
    cleaned = re.sub(r'^[:\-\s]+', '', cleaned)
    # Replace various separators with slashes
...
def normalize_month_text_date(s: str) -> str:
    ...
    # DD MMM YY or DD MMM YYYY  (e.g., '17 SEP 97' or '17 SEP 1997')
    m = re.search(
        r'^\s*(\d{1,2})\s+'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
        r'\s+(\d{2,4})(?:st|nd|rd|th)?\s*$', t, re.I
    )
    if m:
        dd = int(m.group(1))
        mm = _mm_from_month_name(m.group(2))
...
def extract_arp_filed_date(t: str) -> str:
    ...
    Tries to recognize clerk stamps such as:
      Type 1: 'FILED FOR RECORD' + '22 January 2008' on next line (DD Month YYYY)
      Type 2: 'Filed: 03/28/2020 3:41' (MM/DD/YYYY HH:MM on same line)
      Also handles OCR misreads like 'FEED FOR RECORD', 'FED FOR RECORD', etc.
    Returns MM/DD/YYYY or "".
    Note: Ignores "Updated" dates as they are just form dates.
    """
    if not t: return ""

    # Pattern for file stamp keywords (including OCR misreads)
    # FILED/Filed/FEED/FED + FOR RECORD, or just Filed:/Entered:
    filed_pattern = r'(?:F[IE][LE]D|FEED|FED)(?:\s+(?:FOR|FDR|F0R)\s+(?:RECORD|REC0RD|RECDRD))?|Entered(?:\s+for\s+Record)?'

    # Priority 1: 'Filed: MM/DD/YYYY HH:MM' format (clearer stamp on ORDER files)
    # Example: "Filed: 03/28/2020 3:41" -> 03/28/2020
    # Note: Strip time portion, only keep date
    m = re.search(
    ...
    if m:
        date_part = m.group(1)
        # Ensure MM/DD/YYYY format (pad single digits)
        parts = date_part.split('/')
        if len(parts) == 3:
            mm, dd, yyyy = parts
            return f"{int(mm):02d}/{int(dd):02d}/{yyyy}"

    # Priority 2: 'DD Month YYYY' format near file stamp (Type 1 stamp)
    # Example: "FILED FOR RECORD\n22 January 2008" -> 01/22/2008
    # This is the most common format on ARPs
    m = re.search(
        rf'({filed_pattern})\b.{{0,80}}?(\d{{1,2}})\s+'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
        r'\s+(\d{{4}})',
        t, re.I|re.S
    ...
    if m:
        ...
        yyyy = m.group(4)
        out = _fmt_mdY(mm, dd, yyyy)
        if out: return out

    # Priority 3: 'YY MON DD' format (2-digit year, for older handwritten stamps)
    # Example: "FEED FOR RECORD\n20 MAR 28" -> 03/28/2020
    m = re.search(
        rf'({filed_pattern})\b.{{0,80}}?(\d{{2}})\s+'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
        r'\s+(\d{{1,2}})(?:\s+[AP]\s+\d)?',  # Optional time like "P 3 41"
        t, re.I|re.S
    )
    if m:
        yy = int(m.group(2))
    ...
    # Priority 4: Numeric MM/DD/YYYY near 'Filed/Entered'
    m = re.search(rf'({filed_pattern})\b.{{0,60}}?(\d{{1,2}}[./-]\d{{1,2}}[./-]\d{{2,4}})', t, re.I|re.S)
    if m:
    ...
    # Priority 5: 'Month 22, 2025' near 'Filed/Entered'
    m = re.search(
        rf'({filed_pattern})\b.{{0,60}}?'
        r'((Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
    ...
    # Priority 6: 'YYYY Mon 22' near 'Filed/Entered'
    m = re.search(
        rf'({filed_pattern})\b.{{0,60}}?(\d{{4}})\s+'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
    ...
    # Priority 7: 'Mon 22 2025' near 'Filed/Entered'
    m = re.search(
        rf'({filed_pattern})\b.{{0,60}}?'
        r'(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|'
        r'Aug(?:ust)?|Sep(?:t)?(?:ember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)'
    ...
    # Last resort: anywhere 'DD Mon YYYY' (but NOT if preceded by "Updated")
    # This catches file stamps that were misread entirely
    for m in re.finditer(
        ...
        # Skip if "Updated" appears within 30 chars before this date
        context_start = max(0, m.start() - 30)
        context = t[context_start:m.start()]
        if 'updated' in context.lower():
            continue
        dd = int(m.group(1))
        mm = _mm_from_month_name(m.group(2))
        yyyy = m.group(3)
...
def clean_ocr_underscores(text: str) -> str:
    ...
    # Pattern: _char_char_char_ -> charcharchar
    ...
    try:
        ...
        result = re.sub(r'_([a-zA-Z0-9])_([a-zA-Z0-9])_([a-zA-Z0-9])_', r'\g<1>\g<2>\g<3>', text)
        result = re.sub(r'_([a-zA-Z0-9])_([a-zA-Z0-9])_', r'\g<1>\g<2>', result)
        result = re.sub(r'_([a-zA-Z0-9])_', r'\g<1>', result)
        ...
        result = re.sub(r'_([a-zA-Z0-9])', r'\g<1>', result)
        result = re.sub(r'([a-zA-Z0-9])_', r'\g<1>', result)

        return result
    except (re.error, IndexError) as e:
        # If regex fails for any reason, return the original text
        log(f"  WARNING: clean_ocr_underscores failed: {e}")
        return text

def _looks_like_two_addresses(s: str) -> bool:
    """
    Enhanced heuristic: return True if the string likely contains two separate addresses.
    Signals:
      - 2+ ZIP codes (##### or #####-####)
      - 2+ street-type tokens *and* a separator like ';', '/', or ' and '
      - 2+ state abbreviations (TX, CA, etc.)
      - Multiple street numbers (indicating separate addresses)
    """
    if not s:
        return False

    # ZIP codes - most reliable indicator
    zip_codes = re.findall(r'\b\d{5}(?:-\d{4})?\b', s)
    if len(zip_codes) >= 2:
        return True

    # Street numbers - if we see multiple street numbers, likely separate addresses
    street_numbers = re.findall(r'\b\d{1,5}\s+(?:[A-Za-z]|St|Street|Ave|Avenue|Rd|Road|Dr|Drive)', s)
    ...
    if len(street_tokens) >= 2 and (';' in s or ' / ' in s or ' and ' in s.lower() or '&' in s):
        return True
...
def _split_two_addresses(address_text: str) -> list[str]:
    """
    Attempt to split a concatenated address string into two separate addresses.
    Returns a list of 1-2 addresses.
    """
    if not address_text or not _looks_like_two_addresses(address_text):
        return [address_text] if address_text else []
    
    # Common separators that might separate two addresses
    separators = [';', ' / ', ' and ', ' & ', '\n', '|']
    
    for sep in separators:
        if sep in address_text:
            parts = address_text.split(sep, 1)
            if len(parts) == 2:
                addr1, addr2 = parts[0].strip(), parts[1].strip()
                # Validate that both parts look like addresses
                if (re.search(r'\d', addr1) and re.search(r'\d', addr2) and 
                    (re.search(r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace)\b', addr1, re.I) or
                     re.search(r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace)\b', addr2, re.I))):
                    return [addr1, addr2]
    
    # If no separator found, try to split by ZIP codes
    zip_matches = list(re.finditer(r'\b\d{5}(?:-\d{4})?\b', address_text))
    if len(zip_matches) >= 2:
        # Split at the first ZIP code
        first_zip_end = zip_matches[0].end()
        addr1 = address_text[:first_zip_end].strip()
        addr2 = address_text[first_zip_end:].strip()
        if addr1 and addr2:
            return [addr1, addr2]
    
    return [address_text]

def _is_complete_address(address: str) -> bool:
    """
    Very flexible address validation to capture more addresses.
    Since g2 address is 100% complete (mirroring works), focus on capturing
    more guardian1 addresses even if they're not perfect.
    """
    if not address or len(address.strip()) < 5:
        return False
    
    # Must have street number
    if not re.search(r'\b\d{1,5}\b', address):
        return False
    
    # Must have street type OR city/state indicators
    has_street_type = re.search(r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace|Way|Loop|Pass|Cove|Circle|Cir|Hwy|Highway)\b', address, re.I)
    has_city_state = re.search(r'\b[A-Za-z]{2,}\b', address) and (',' in address or re.search(r'\b\d{5}(?:-\d{4})?\b', address))
    
    if not (has_street_type or has_city_state):
        return False
    
    # Count available components (city, state, ZIP)
    components = 0
    if re.search(r'\b\d{5}(?:-\d{4})?\b', address):  # ZIP code
        components += 1
    if re.search(r'\b[A-Za-z]{2,}\b', address):  # State (2+ letters)
        components += 1
    if ',' in address:  # City, state structure
        components += 1
    
    # Need at least 1 of 3 components (very flexible)
    return components >= 1
...
def clean_address(raw: str) -> str:
    """
    Enhanced address cleaning with better OCR error recovery.
    ...
    # Enhanced OCR error recovery:
    # - Insert a space between a leading house number and the street name (e.g., '101Acapulco' -> '101 Acapulco')
    s = re.sub(r'^(\d{1,6})([A-Za-z])', r'\g<1> \g<2>', s)
    # - Insert spaces between camel-cased tokens (e.g., 'AcapulcoCourt' -> 'Acapulco Court')
    s = re.sub(r'([a-z])([A-Z])', r'\g<1> \g<2>', s)
    # - Fix common OCR substitutions (0->O, 1->I, 5->S, etc.)
    s = re.sub(r'\b(\d+)\s*([A-Za-z])', r'\g<1> \g<2>', s)  # Number followed by letter
    # - Normalize punctuation before state abbreviations (e.g., 'Austin. TX' -> 'Austin, TX')
    s = re.sub(r'([A-Za-z])\.[ ]+([A-Z]{2}\b)', r'\g<1>, \g<2>', s)
    # - Fix missing spaces around commas
    s = re.sub(r'([A-Za-z])(,)([A-Za-z])', r'\g<1>, \g<3>', s)
    # - Fix common OCR errors in street types
    s = re.sub(r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace|Way|Loop|Pass|Cove|Circle|Cir|Hwy|Highway)\b',
               lambda m: m.group(1).title(), s, flags=re.IGNORECASE)

    # Additional OCR error fixes:
    # - Fix ZIP code OCR errors (e.g., "7874.8" -> "78748")
    s = re.sub(r'\b(\d{5})\.(\d)\b', r'\g<1>\g<2>', s)
    # - Fix missing spaces in street names
    s = re.sub(r'([A-Za-z])(\d)', r'\g<1> \g<2>', s)  # Letter followed by number
    s = re.sub(r'(\d)([A-Za-z])', r'\g<1> \g<2>', s)  # Number followed by letter
    # - Fix common OCR character substitutions
    s = re.sub(r'\b0(\d{4})\b', r'\g<1>', s)  # Remove leading zero from ZIP if OCR added it
    # - Fix state abbreviations
    s = re.sub(r'\bTx\b', 'TX', s, flags=re.I)
    s = re.sub(r'\bTx\.\b', 'TX', s, flags=re.I)

    # Enhanced OCR fixes for better address capture:
    # - Fix common OCR errors in street numbers (e.g., "1O1" -> "101")
    s = re.sub(r'\b(\d+)[O0](\d+)\b', r'\g<1>O\g<2>', s)  # Fix O/0 confusion in numbers
    # - Fix missing spaces in compound street names (e.g., "NorthMain" -> "North Main")
    s = re.sub(r'([A-Z][a-z]+)([A-Z][a-z]+)', r'\g<1> \g<2>', s)
    # - Fix ZIP code with missing digits (e.g., "7874" -> "78748" if it looks like a ZIP)
    if re.search(r'\b\d{4}\b', s) and not re.search(r'\b\d{5}\b', s):
        s = re.sub(r'\b(\d{4})\b', r'\g<1>' + '0', s)  # Add trailing zero to 4-digit numbers that might be ZIPs
    
    # - Fix common OCR errors in city names (e.g., "Austln" -> "Austin")
    s = re.sub(r'\bAustln\b', 'Austin', s, flags=re.I)
    s = re.sub(r'\bAustln\b', 'Austin', s, flags=re.I)
    # - Fix common OCR errors in state names
    s = re.sub(r'\bT[eE]xas\b', 'Texas', s, flags=re.I)
    s = re.sub(r'\bT[eE]x\b', 'TX', s, flags=re.I)
...
def parse_liveswith_guardian(text: str) -> str | None:
    """
    Checkbox-only reader for: '4. Do you live with the ward?  [ ] YES  [ ] NO'
    Returns:
    ...
    if not text:
        return None
    # Match with optional question number prefix (e.g., "4. Do you live...")
    m = re.search(r'(?:\d+[\.\)]\s*)?Do\s+you\s+(?:reside|live)\s+with\s+the\s+ward', text, re.I)
    if not m:
        return None
    window = text[m.start(): m.start() + 250]

    # Replace various checkbox characters with 'X'
    for ch in ['☑','☒','■','█','✔','[OK]','[OK]','❎','[FAIL]','✘','[X]','⊠','☓','⨯']:
        window = window.replace(ch, 'X')

    # Normalize various checkbox patterns to 'X'
    window = re.sub(r'\[(?:x|X)\]', '[X]', window, flags=re.I)
    window = re.sub(r'\((?:x|X)\)', '(X)', window, flags=re.I)
    window = re.sub(r'\[\s*[xX]\s*\]', '[X]', window)  # [ x ] or [ X ]
    window = re.sub(r'\(\s*[xX]\s*\)', '(X)', window)  # ( x ) or ( X )

    # Also look for patterns like "X YES" or "YES X" or "[X] YES" or "YES [X]"
    def _checked(word: str) -> bool:
        # Look for X near the word (before or after, with optional brackets/parens)
        patterns = [
            rf'X\s*{word}\b',           # X YES
            rf'\b{word}\s*X',           # YES X
            rf'\[X\]\s*{word}\b',       # [X] YES
            ...
            rf'\(X\)\s*{word}\b',       # (X) YES
            rf'\b{word}\s*\(X\)',       # YES (X)
            rf'[▪●•]\s*{word}\b',       # Bullet point YES
            rf'\b{word}\s*[▪●•]',       # YES bullet
        ]
...
def capture_address_after_label(text: str, label_pattern: str, max_lines: int = 3) -> str:
    ...
    for ln in cleaned_lines:
        if re.search(r'\d', ln) and re.search(
            r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace|Way|Loop|Pass|Cove|Circle|Cir|Hwy|Highway)\b',
            ln, re.IGNORECASE
        ):
            return clean_address(ln)
    
    # 6) Enhanced fallback: try to stitch partial address components
    street_candidates = []
    city_candidates = []
    
    for ln in cleaned_lines:
        # Look for street-like lines (has number and street type)
        if re.search(r'\d', ln) and re.search(
            r'\b(St|Street|Ave|Avenue|Rd|Road|Dr|Drive|Ln|Lane|Blvd|Boulevard|Ct|Court|Pl|Place|Pkwy|Parkway|Trl|Trail|Ter|Terrace|Way|Loop|Pass|Cove|Circle|Cir|Hwy|Highway)\b',
            ln, re.IGNORECASE
        ):
            street_candidates.append(ln)
        
        # Look for city/state/zip-like lines
        if re.search(r'\b[A-Za-z]{2,}\b', ln) and (re.search(r'\b\d{5}(?:-\d{4})?\b', ln) or re.search(r'\b[A-Z]{2}\b', ln)):
            city_candidates.append(ln)
    
    # Try to combine the best street and city candidates
    if street_candidates and city_candidates:
        best_street = max(street_candidates, key=len)  # Prefer longer street names
        best_city = max(city_candidates, key=len)      # Prefer longer city names
        combined = f"{best_street}, {best_city}"
        cleaned = clean_address(combined)
        if _is_complete_address(cleaned):
            return cleaned
        else:
            return cleaned  # Return even if incomplete
...
def improve_mapping(data: dict) -> dict:
    ...
    for k in ("gemail", "g2eamil"):
        raw = data.get(k, "").strip()
        # Remove OCR noise prefixes like "66Email", "Email", etc.
        raw = re.sub(r'^[\d\s]*[Ee]mail\s*', '', raw)
    ...
    for k in ("guardian1", "Guardian2", "wardfirst", "wardlast"):
        ...
        # Remove "I, " prefix from guardian names (OCR artifact)
        v = re.sub(r'^I,\s+', '', v, flags=re.I)
    ...
    # Fix OCR misread: "Pack" -> "Park" when ward last name is "Park"
    # This is a context-aware fix for family guardianships
    wardlast = data.get("wardlast", "")
    if "Park" in wardlast:
        for k in ("guardian1", "Guardian2"):
            guardian_name = data.get(k, "")
            if guardian_name and "Pack" in guardian_name:
                data[k] = guardian_name.replace("Pack", "Park")
                log(f"  OCR fix: Changed 'Pack' -> 'Park' in {k} (context: wardlast={wardlast})")
    ...
    # Enhanced shared address logic for guardians
    if data.get("Guardian2") and not data.get("g2 address") and data.get("gaddress"):
        if not _looks_like_two_addresses(data["gaddress"]):
            ...
            # Try to split the addresses if they appear to be concatenated
            split_addresses = _split_two_addresses(data["gaddress"])
            if len(split_addresses) == 2:
                data["gaddress"] = split_addresses[0]
                data["g2 address"] = split_addresses[1]
                log("  Split concatenated addresses into separate guardian addresses.")
    
    # Also check if we have g2 address but no gaddress (reverse case)
    if data.get("Guardian2") and not data.get("gaddress") and data.get("g2 address"):
        if not _looks_like_two_addresses(data["g2 address"]):
            data["gaddress"] = data["g2 address"]
            log("  Mirrored g2 address -> gaddress (single/shared address detected).")
...
def parse_arp_fields(text: str, order_ward_names: dict[str, dict[str, str]] | None = None) -> dict | None:
    """
    ...
    order_ward_names: Optional dict of {causeno: {'first': '', 'middle': '', 'last': ''}} for fallback.
    """
    t = normalize_ocr_confusions(text)
    ...
    # FALLBACK: If ARP ward name extraction failed or returned empty, use ORDER ward names
    if order_ward_names and (not data.get("wardfirst") or not data.get("wardlast")) and cause in order_ward_names:
        order_names = order_ward_names[cause]
        data["wardfirst"] = order_names.get('first', '')
        data["wardmiddle"] = order_names.get('middle', '')
        data["wardlast"] = order_names.get('last', '')
        log(f"  Using ORDER ward name fallback for {cause}: {data['wardfirst']} {data['wardmiddle']} {data['wardlast']}")
    ...
    # Try to extract phone from "Phone" label - use more robust extraction
    wtele_raw = safe_after_label(t, r'(Ward\s*Phone|Phone)', "phone")
    # If safe_after_label fails, try direct phone number search near "Phone" label
    if not wtele_raw or not PHONE_RE.search(wtele_raw):
        # Look for "Phone" label and grab phone number from same line or next line
        m = re.search(r'Phone\s*[:\-]?\s*(.{0,50})', t, re.I)
        if m:
            context = m.group(1)
            phone_match = PHONE_RE.search(context)
            if phone_match:
                wtele_raw = phone_match.group(0)
    data["wtele"] = normalize_phone(wtele_raw)
    # Enhanced ward DOB extraction - handle all variations
    wdob = ""
    
    # Try the original method first
    wdob = safe_after_label(t, r'(Ward\s*DOB|DOB|Date\s*of\s*Birth)', "date")
    
    # If that fails, try more flexible patterns
    if not wdob:
        # Look for DOB variations in ward section
        ward_section = _slice_between(t, r'(Ward\s*Information|Ward\s*Name|Ward\s*:\s*Name|1\.\s*WARD\b)', r'(Guardian\(s\)|Guardian\s*Information|2\.\s*GUARDIAN\(s\)\b)', max_len=800)
        if ward_section:
            # Try various DOB label patterns - comprehensive date format variations
            dob_patterns = [
                # Standard numeric formats: MM/DD/YYYY, MM-DD-YYYY, MM.DD.YYYY
                r'(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}[./\-\[\]]?[0-9]{1,2}[./\-\[\]]?[0-9]{2,4})',
                # Age/DOB combinations
                r'(?:Age\s+\d+[,\s]*)?(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}[./\-\[\]]?[0-9]{1,2}[./\-\[\]]?[0-9]{2,4})',
                # Month name formats: Sept 9, 2000; 9 Sept 2000; September 9, 2000; 9 September 2000
                r'(?:DOB|dob|D0B|DOB:|dob:)\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+[0-9]{1,2}[,\s]+[0-9]{2,4})',
                r'(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+[0-9]{2,4})',
                # Age/DOB with month names
                r'(?:Age\s+\d+[,\s]*)?(?:DOB|dob|D0B|DOB:|dob:)\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+[0-9]{1,2}[,\s]+[0-9]{2,4})',
                r'(?:Age\s+\d+[,\s]*)?(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+[0-9]{2,4})',
                # Handwritten variations with OCR artifacts
                r'(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}[./\-\[\]\\|]+[0-9]{1,2}[./\-\[\]\\|]+[0-9]{2,4})',
                # Very flexible pattern for any date-like format after DOB
                r'(?:DOB|dob|D0B|DOB:|dob:)\s*([0-9]{1,2}[^a-zA-Z]{1,3}[0-9]{1,2}[^a-zA-Z]{1,3}[0-9]{2,4})'
            ]
            
            for pattern in dob_patterns:
                match = re.search(pattern, ward_section, re.I)
                if match:
                    wdob = match.group(1)
                    break
    
    # If still no DOB found, look for any date pattern in ward section
    if not wdob:
        ward_section = _slice_between(t, r'(Ward\s*Information|Ward\s*Name|Ward\s*:\s*Name|1\.\s*WARD\b)', r'(Guardian\(s\)|Guardian\s*Information|2\.\s*GUARDIAN\(s\)\b)', max_len=800)
        if ward_section:
            # Look for any date pattern that looks like a birth date (reasonable year range)
            date_match = re.search(r'\b(19[0-9]{2}|20[0-2][0-9])\b', ward_section)
            if date_match:
                # Find the full date pattern around this year
                year = date_match.group(1)
                full_date_pattern = r'\b([0-9]{1,2}[./\-\[\]]?[0-9]{1,2}[./\-\[\]]?' + re.escape(year) + r')\b'
                full_match = re.search(full_date_pattern, ward_section)
                if full_match:
                    wdob = full_match.group(1)
    
    data["wdob"] = normalize_month_text_date(to_slashes(wdob))

    # --- Liveswith: checkbox-only logic (YES->Guardian; NO->blank; both->Guardian; unknown->blank) ---
    lw = parse_liveswith_guardian(t)
    log(f"  Liveswith detection: result={lw}, text_length={len(t)}")
    if lw is None:
        # Debug: Check if the question exists in the text at all
        m = re.search(r'Do\s+you\s+(?:reside|live)\s+with', t, re.I)
        if m:
            # Show what the OCR actually sees around the question
            context_start = max(0, m.start())
            context_end = min(len(t), m.end() + 200)
            context = t[context_start:context_end]
            # Replace newlines for logging
            context_clean = context.replace('\n', ' ↵ ')[:250]
            log(f"  WARNING: Found question but no checkbox detected")
            log(f"  OCR context: ...{context_clean}...")
        else:
            log(f"  WARNING: 'Do you live with the ward' question NOT FOUND in OCR text")
    ...
    # Clean up any GUARDIAN text that got mixed into ward address
    if data["waddress"]:
        data["waddress"] = re.sub(r',?\s*\d+\.\s*GUARDIAN\(s\)\s*:.*$', '', data["waddress"], flags=re.IGNORECASE).strip()
    ...
    # Enhanced address contamination cleanup - remove guardian info from address fields
    if data.get("gaddress"):
        ...
        # REMOVED: Line 2634 was too aggressive - it removed street names like "Monarch Dr" thinking they were person names
        # Only remove names if they appear at the START of the address (before any numbers)
        # This preserves street names like "Monarch Drive", "Bella montagna Circle", etc.
        if re.match(r'^[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\s*,?\s*\d', data["gaddress"]):
            # Name appears before street number - likely a person name, remove it
            data["gaddress"] = re.sub(r'^[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\s*,?\s*', '', data["gaddress"]).strip()
        # Remove common guardian-related text
        data["gaddress"] = re.sub(r'\b(?:guardian|guardians|name|names|phone|email|dob|relationship)\b[:\s]*', '', data["gaddress"], flags=re.I).strip()
    ...
    name_line = None
    name_line_index = -1
    for i, line in enumerate(gslice):
        ...
        if (re.search(r'name(?:\(s\))?s?\s*(?:[:\-]|\s)', line, re.I) or
            ...
            re.search(r'nam(?:ico?|e)?\)\s*', line, re.I) or  # OCR error: "Namico)", "Nami)", "Nam)" instead of "Name(s)"
    ...
    if name_line:
        ...
        # ENHANCED: Check line BEFORE Name(s) for Guardian2 name (Brener format)
        # Some ARPs have Guardian2 name on its own line before "Name(s) Guardian1 name"
        if g1 and not g2 and name_line_index > 0:
            prev_line = gslice[name_line_index - 1].strip()
            # Check if previous line looks like a standalone human name (no labels, no punctuation except spaces)
            if (prev_line and
                _looks_like_human_name(prev_line) and
                not re.search(r'name\(s\)|guardian|address|phone|email|age|dob|relationship|ward', prev_line, re.I)):
                # This could be Guardian2's name
                log(f"Found potential Guardian2 name on line before Name(s): {prev_line}")
                g2 = prev_line
    ...
    # PROTECTED FIELD: guardian1 is at 100% - DO NOT OVERWRITE if already populated
    if g1 and _looks_like_human_name(g1) and not data.get("guardian1"):
        data["guardian1"] = g1
    elif g1:
        pass  # g1 rejected by _looks_like_human_name
    
    # Clean up organization suffixes from g2 BEFORE validation (e.g., "Inc. - John Doe" -> "John Doe")
    if g2:
        g2_cleaned = re.sub(r'^(Inc\.|LLC|Corp\.|Co\.|Ltd\.?)\s*[-–—]\s*', '', g2, flags=re.IGNORECASE).strip()
        g2 = g2_cleaned if g2_cleaned else g2
    ...
    if not data.get("guardian1") and not data.get("Guardian2"):
        ...
        for line in gslice:
            # Skip lines that are clearly not names OR form text
            if re.search(r'(?:age|dob|email|phone|address|relationship|city|state|zip|branch|certification|commission|investigation|disability|services|department|convicted|felony|misdemeanor|traffic|offense)', line, re.I):
                continue
    ...
    try:
        ...
        # SIMPLIFIED EMAIL EXTRACTION: Scan entire guardian section for all @ signs
        # Join all guardian lines into one text block
        guardian_text = '\n'.join(gslice)
        # Find ALL email addresses in the entire guardian section
        all_emails = re.findall(r'[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}', guardian_text)

        # FALLBACK: If no emails found, try removing whitespace/newlines (handles OCR line breaks)
        if not all_emails:
            # Remove ALL whitespace including newlines, then look for @ signs
            guardian_text_compact = re.sub(r'\s+', '', guardian_text)
            # Try to find email patterns in the compacted text
            all_emails = re.findall(r'[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}', guardian_text_compact)

        if all_emails:
            if len(all_emails) >= 2:
                # First @ goes to guardian1, second @ goes to guardian2
                guardian_fields['email'] = f"{all_emails[0]} / {all_emails[1]}"
            elif len(all_emails) == 1:
                # Only one email found - assign to guardian1
                guardian_fields['email_single'] = all_emails[0]
        ...
        for line in gslice:
            ...
            if 'dob' in line_lower:
                ...
                if dob_match:
                    ...
                    # NEW: Convert period-formatted dates to slash format (2.8.1960 -> 2/8/1960)
                    cleaned_dob_value = re.sub(r'(\d{1,2})\.(\d{1,2})\.(\d{2,4})', r'\1/\2/\3', cleaned_dob_value)

                    # Extract the date part(s) - look for multiple dates separated by spaces, commas, or slashes
                    # Pattern like "8/16/65 4/15/65" or "8/16/65, 4/15/65"
                    ...
                    if date_part:
                        ...
                        # Check if it contains multiple dates with clear separators
                            ...
                            guardian_fields['dob'] = clean_dob
                        # NEW: Check for comma-separated dates (like "5/20/65, 8/24/64")
                        elif re.search(r'\d{1,2}/\d{1,2}/\d{2,4}\s*,\s*\d{1,2}/\d{1,2}/\d{2,4}', clean_dob):
                            guardian_fields['dob'] = clean_dob
...
def _slice_guardian_section(t: str) -> list[str]:
    ...
    # FALLBACK: If no guardian anchor found, look for guardian info after WARD section
    # Some forms (esp. Document AI) don't have explicit "GUARDIAN:" header
    # Pattern: WARD section ends with "New Address?" followed by guardian Name line
    if not anchor:
        # Look for ward section end marker + guardian name pattern
        # Example: "New Address? □ YES ENO" followed by "Namico) DERRICK A STOVEY..."
        fallback_anchor = re.search(
            r'new\s*address\?\s*[□☐]?\s*yes\s*[□☐]?\s*[en]?no',
            t, re.IGNORECASE
        )
        if fallback_anchor:
    ...
    # ENHANCED: Check if line BEFORE anchor contains Guardian2 name
    # Some ARPs (Brener format) have Guardian2 name on standalone line before "2. GUARDIAN (s): Name(s)"
    # Example: "Lazaro Brener\n2. GUARDIAN (s): Name(s) Fay Brener..."
    pre_anchor_line = None
    anchor_start = anchor.start()
    if anchor_start > 0:
        # Find the line before the anchor
        lines_before_anchor = t[:anchor_start].splitlines()
        if lines_before_anchor:
            prev_line = lines_before_anchor[-1].strip()
            # Check if it's a standalone human name (not a label or instruction)
            if (prev_line and
                len(prev_line.split()) >= 2 and  # At least first + last name
                not re.search(r'name\(s\)|guardian|address|phone|email|age|dob|relationship|ward|yes|no|new|co-guard', prev_line, re.I) and
                re.search(r'^[A-Z][a-z]+\s+[A-Z][a-z]+', prev_line)):  # Looks like "First Last"
                pre_anchor_line = prev_line

    # End: next numbered section (3.), Visit Date/Time, or Cause No, or "During the past"
    m_end = re.search(r'\n\s*\d+\.\s|visit\s*date|visit\s*time|cause\s*no|during\s*the\s*past', t[start:], re.IGNORECASE)
    ...
    # Prepend the pre-anchor line if found (Guardian2 name appearing before anchor)
    if pre_anchor_line:
        raw_lines.insert(0, pre_anchor_line)
    ...
    for ln in raw_lines:
        ...
        if _INSTR_LINE_RE.search(ln):
            ...
            # e.g., "Name(s) Magdalena Wolk" or "Namico) DERRICK A STOVEY"
...
def _clean_extracted_name(n: str | None) -> str | None:
    ...
    # OCR corrections for common name errors
    # Fix STOVEY/STONEY -> TONEY (OCR confusion between ST/T)
    n2 = re.sub(r'\bSTO(?:VEY|NEY)\b', 'TONEY', n2, flags=re.I)
    n2 = re.sub(r'\bSto(?:vey|ney)\b', 'Toney', n2)
...
def _looks_like_human_name(s: str) -> bool:
    ...
    # Allow all-caps names (like "MEENU JAIN")
    # Allow names with internal capitals (like "LeBoeuf", "McDonald", "O'Brien")
    titled = sum(
        1 for w in words
        if (re.match(r"[A-Z][a-z]+(?:[.\-'][A-Za-z]+)?$", w) is not None or  # Standard: Laura, Brien
            re.match(r"^[A-Z][a-z]*[A-Z][a-z]+$", w) is not None or  # Internal capitals: LeBoeuf, McDonald
            re.match(r"^[A-Z]{2,}$", w) is not None or  # All caps names like "JAIN", "MEENU"
            re.match(r"^(Jr\.?|Sr\.?|III|IV|V)$", w, re.I) is not None)
    )
    if len(words) == 1 and titled < 1:  # Single name needs at least 1 TitleCase or all-caps
...
def _split_guardian_field_by_separators(value: str) -> tuple[str, str]:
    ...
    # Special case for emails: if value contains email patterns, extract them directly
    # Pattern like "email1@domain.com / email2@domain.com" or "email1@domain.com & email2@domain.com"
    # This MUST come before separator-based splitting to prevent '@' from being used as a separator
    if re.search(r'[^@\s]+@[^@\s]+\.[^@\s]+', str(value)):
...
def _extract_guardian_names_from_name_line(name_line: str) -> tuple[str | None, str | None]:
    ...
    # Strip the "Name(s)" label - handle OCR errors like "Name (0)", "Namico)", "Nam", etc.
    ...
    clean_line = re.sub(r'^\s*nam(?:ico?|e)?\)?\s*[:\-]?\s*', '', clean_line, flags=re.I)  # Handle "Namico)", "Nam)", "Nami)"
    ...
    # OCR corrections for common name errors before processing
    # Fix STOVEY/STONEY -> TONEY (OCR confusion between ST/T)
    clean_line = re.sub(r'\bSTO(?:VEY|NEY)\b', 'TONEY', clean_line, flags=re.I)
    clean_line = re.sub(r'\bSto(?:vey|ney)\b', 'Toney', clean_line)
...
def extract_file_suffix(filename: str) -> str:
    """
    Extract numeric suffix from filename for pairing ORDER with ARP.

    Examples:
        "order.pdf" or "ARP.pdf" -> ""
        "order_6.pdf" or "ARP_6.pdf" -> "6"
        "order (6).pdf" or "ARP (6).pdf" -> "6"
        "order6.pdf" or "ARP6.pdf" -> "6"
    ...
    # Extract number from various patterns: order_6, order (6), order6, etc.
    patterns = [
        r'(?:order|arp)\s*[_\-\s]?\(?(\d+)\)?',  # order_6, order (6), order 6
        r'(?:order|arp)(\d+)',  # order6
    ]
    ...
    # No number found - this is the base file (order.pdf, ARP.pdf)
    return ""
...
if __name__ == "__main__":
    ...
    # Pre-pass: collect ORDER cause numbers and ward names to help correct ARP mis-reads
    ORDER_CAUSES: set[str] = set()
    ORDER_WARD_NAMES: dict[str, dict[str, str]] = {}  # {causeno: {'first': '', 'middle': '', 'last': ''}}
    ORDER_FILE_SUFFIX_MAP: dict[str, str] = {}  # {file_suffix: causeno} for filename pairing
    try:
        for f in files:
            ...
            if "order" in nm:
                try:
                    ...
                    if row_hint and row_hint.get("causeno"):
                        causeno = row_hint["causeno"]
                        ORDER_CAUSES.add(causeno)

                        # Store filename suffix for pairing with ARP
                        file_suffix = extract_file_suffix(f.get("name", ""))
                        ORDER_FILE_SUFFIX_MAP[file_suffix] = causeno
                        log(f"  ORDER file suffix '{file_suffix}' -> cause {causeno}")

                        # Store ward names from ORDER if available
                        if row_hint.get("wardfirst") and row_hint.get("wardlast"):
                            ORDER_WARD_NAMES[causeno] = {
                                'first': row_hint.get("wardfirst", ""),
                                'middle': row_hint.get("wardmiddle", ""),
                                'last': row_hint.get("wardlast", "")
                            }
                            log(f"  Stored ORDER ward name for {causeno}: {row_hint.get('wardfirst')} {row_hint.get('wardmiddle')} {row_hint.get('wardlast')}")
                except Exception:
                    pass
        if ORDER_CAUSES:
            log(f"ORDER causes seen this run: {sorted(ORDER_CAUSES)}")
        if ORDER_WARD_NAMES:
            log(f"ORDER ward names collected for {len(ORDER_WARD_NAMES)} cases")
    except Exception as _:
        pass
        ...
        try:
            ...
            if "order" in lower_name:
                ...
                if row and row.get("Dateappointed") and row.get("causeno"):
                    ...
                    # IMPORTANT: Only include ward name fields if they're actually populated
                    # to avoid overwriting good ARP data with empty values from ORDER
                    order_update = {"causeno": row["causeno"], "Dateappointed": row["Dateappointed"]}
                    # Only add ward names if they're non-empty
                    if row.get("wardfirst"):
                        order_update["wardfirst"] = row["wardfirst"]
                    if row.get("wardmiddle"):
                        order_update["wardmiddle"] = row["wardmiddle"]
                    if row.get("wardlast"):
                        order_update["wardlast"] = row["wardlast"]
                    log(f"  Row to upsert (non-empty): {order_update}")
                    upsert_row_to_excel(order_update)
                    log("  [OK] Upserted Dateappointed from ORDER (by causeno).")
                else:
            ...
            if "arp" in lower_name:
                ...
                if len(text or "") < 80:
                    ...
                    # COST-EFFICIENT ORDER: Try free Tesseract first before expensive cloud APIs
                    # If still no text, try Tesseract OCR (FREE, local, 85-95% on typed, 40-60% on handwriting)
                    if len(text or "") < 80:
                        log("  Still little text; trying Tesseract OCR (first 2 pages)...")
                        # Use the new multi-page OCR function for better performance
                        text = extract_text_with_ocr_for_arp_multipage(fpath, max_pages=2)
                        if not text or len(text) < 50:
                            # Fallback to single page OCR if multi-page fails
                            log("  Multi-page OCR failed; trying single page OCR...")
                            t4 = extract_text_with_ocr_for_arp(fpath, psm=4)
                            t6 = extract_text_with_ocr_for_arp(fpath, psm=6)
                            text = t4 if guardian_signal_score(t4) >= guardian_signal_score(t6) else t6
                            log(f"  Single page ARP OCR used {'psm 4' if text==t4 else 'psm 6'} (chars={len(text)})")
                        else:
                            log(f"  Multi-page ARP OCR extracted {len(text)} characters from first 2 pages")

                    # If Tesseract still didn't get much, try Vision API (costs $, but good for handwriting 85-90%)
                    if len(text or "") < 80:
                        log("  Tesseract didn't get much text; trying Vision API...")
                        try:
                    ...
                    # Last resort: Document AI (costs $$, but best for handwriting 90-95%)
                    if len(text or "") < 80:
                        log("  Vision API didn't help; trying Document AI (best for handwriting)...")
                        try:
                            docai_text = extract_text_with_document_ai(pdf_bytes)
                            if docai_text and len(docai_text) > len(text or ""):
                                text = docai_text
                                log(f"  Document AI extracted {len(text)} characters")
                        except Exception as e:
                            log(f"  Document AI failed: {e}")

                # First parse pass
                row = parse_arp_fields(text, ORDER_WARD_NAMES)

                # If guardians missing, try Document AI first (best for handwriting)
                if (row is None) or (not row.get("guardian1") and not row.get("Guardian2")):
                    log("  Anchored parse weak; trying Document AI for handwriting...")
                    try:
                        text_docai = extract_text_with_document_ai(pdf_bytes)
                        if text_docai and guardian_signal_score(text_docai) > guardian_signal_score(text):
                            text = text_docai
                            log(f"  Document AI improved guardian detection (chars={len(text)})")
                    except Exception as _e:
                        log(f"  Document AI error ignored: {_e}")

                # If Document AI didn't help, try Vision
                row = parse_arp_fields(text, ORDER_WARD_NAMES)
                if (row is None) or (not row.get("guardian1") and not row.get("Guardian2")):
                    log("  Still weak; trying Vision API...")
                    try:
                        text_v = extract_text_with_vision(pdf_bytes)
                        if text_v and guardian_signal_score(text_v) > guardian_signal_score(text):
                            text = text_v
                            log(f"  Vision API improved guardian detection (chars={len(text)})")
                    except Exception as _e:
                ...
                row = parse_arp_fields(text, ORDER_WARD_NAMES)
                if (row is None) or (not row.get("guardian1") and not row.get("Guardian2")):
                    log("  Still weak; retrying Tesseract with both psm 4/psm 6 and picking best...")
                    try:
                        ...
                        if guardian_signal_score(text_best) >= guardian_signal_score(text):
                            text = text_best
                            log(f"  Tesseract OCR (reselect) chars={len(text)}")
                    except Exception as _e:
                ...
                # Even if parse failed, try to match with ORDER data by filename
                if row is None or not row.get("causeno"):
                    if row is None:
                        log("  Parse failed, creating minimal row for three-tier matching...")
                        row = {}
                    else:
                        log("  ARP missing cause number, attempting three-tier matching...")

                    matched_cause = None

                    # TIER 1: Filename pairing (most reliable)
                    arp_suffix = extract_file_suffix(name)
                    if arp_suffix in ORDER_FILE_SUFFIX_MAP:
                        matched_cause = ORDER_FILE_SUFFIX_MAP[arp_suffix]
                        log(f"  [OK] TIER 1 (Filename): ARP suffix '{arp_suffix}' matched to ORDER cause {matched_cause}")

                    # TIER 2: Ward name matching (fallback)
                    if not matched_cause and row.get("wardlast") and row.get("wardfirst"):
                        arp_name = f"{row.get('wardfirst', '')} {row.get('wardmiddle', '')} {row.get('wardlast', '')}".strip()
                        for causeno, ward_data in ORDER_WARD_NAMES.items():
                            order_name = f"{ward_data.get('first', '')} {ward_data.get('middle', '')} {ward_data.get('last', '')}".strip()
                            if arp_name.lower() == order_name.lower():
                                matched_cause = causeno
                                log(f"  [OK] TIER 2 (Name): ARP ward '{arp_name}' matched to ORDER cause {matched_cause}")
                                break

                    # If matched, use the ORDER's cause number and ward name
                    if matched_cause:
                        row["causeno"] = matched_cause
                        log(f"  [OK] Cause number filled from ORDER: {matched_cause}")

                        # Also populate ward name from ORDER if not already present
                        if matched_cause in ORDER_WARD_NAMES:
                            ward_data = ORDER_WARD_NAMES[matched_cause]
                            if not row.get("wardfirst"):
                                row["wardfirst"] = ward_data.get('first', '')
                            if not row.get("wardmiddle"):
                                row["wardmiddle"] = ward_data.get('middle', '')
                            if not row.get("wardlast"):
                                row["wardlast"] = ward_data.get('last', '')
                            log(f"  [OK] Ward name populated from ORDER: {row.get('wardfirst')} {row.get('wardmiddle')} {row.get('wardlast')}")
                    else:
                        log("  [WARN] No match found in any tier")

                # Final validation: skip only if we have neither cause number nor ward name
                if not row.get("causeno") and not (row.get("wardfirst") or row.get("wardlast")):
                    log("  Skipped - no cause number and no ward name found")
                    continue

                # Align ARP cause with nearest ORDER cause if likely off by one digit (OCR correction)
                if row.get("causeno"):
                    try:
                ...
                log("  [OK] Upserted row in Excel (by causeno).")
                continue
