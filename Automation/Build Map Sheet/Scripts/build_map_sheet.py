...
try:
    ...
    OUTPUT_DOCX = str(_app_paths.APP_ROOT / "Ward_Map_Sheet.docx")
    _CONFIG_DIR = _app_paths.CONFIG_DIR
except Exception:
    # Fallback to hardcoded paths
    EXCEL_PATH = r"C:\GoogleSync\GuardianShip_App\App Data\ward_guardian_info.xlsx"
    OUTPUT_DOCX = r"C:\GoogleSync\GuardianShip_App\Ward_Map_Sheet.docx"
...
GEOCODE_CACHE_CSV = str(_script_dir / "geocode_cache.csv")

# Routing options
# Pull the Google Maps API key from Config folder, then environment variable, then legacy location
GOOGLE_MAPS_API_KEY = ""

# Try app's Config folder first (NEW - self-contained)
config_key_path = str(_CONFIG_DIR / "Keys" / "google_maps_api_key.txt")
if os.path.exists(config_key_path):
    try:
        with open(config_key_path, 'r') as f:
            GOOGLE_MAPS_API_KEY = f.read().strip()
            print(f"[config] Loaded Google Maps API key from Config folder")
    except Exception as e:
        print(f"[config] Could not read {config_key_path}: {e}")

# Fall back to environment variable if not in Config folder
if not GOOGLE_MAPS_API_KEY:
    GOOGLE_MAPS_API_KEY = os.environ.get("GOOGLE_MAPS_API_KEY", "").strip()
    if GOOGLE_MAPS_API_KEY:
        print(f"[config] Loaded Google Maps API key from environment variable")

# Fall back to legacy C:\configlocal location
if not GOOGLE_MAPS_API_KEY:
    legacy_key_path = r"C:\configlocal\Keys\google_maps_api_key.txt"
    if os.path.exists(legacy_key_path):
        try:
            with open(legacy_key_path, 'r') as f:
                GOOGLE_MAPS_API_KEY = f.read().strip()
                print(f"[config] Loaded Google Maps API key from legacy location (C:\\configlocal)")
        except Exception as e:
            print(f"[config] Could not read {legacy_key_path}: {e}")
...
def build_map_image(filtered_df, dpi=200):
    ...
    # Basemap - use Google Static Maps if API key available, else contextily, else plain
    basemap_added = False

    if GOOGLE_MAPS_API_KEY:
        try:
            # Calculate center point
            center_lat = (plot_df["lat"].min() + plot_df["lat"].max()) / 2
            center_lon = (plot_df["lon"].min() + plot_df["lon"].max()) / 2

            # Calculate appropriate zoom level based on bounds
            lat_range = plot_df["lat"].max() - plot_df["lat"].min()
            lon_range = plot_df["lon"].max() - plot_df["lon"].min()
            max_range = max(lat_range, lon_range)

            # Estimate zoom (rough approximation)
            if max_range > 1.0:
                zoom = 9
            elif max_range > 0.5:
                zoom = 10
            ...
            map_url += f"center={center_lat},{center_lon}"
            map_url += f"&zoom={zoom}"
            map_url += f"&size=640x480"
            map_url += f"&maptype=roadmap"
            map_url += f"&key={GOOGLE_MAPS_API_KEY}"
            ...
            if response.status_code == 200:
                ...
                # Get map bounds in lat/lon (approximate)
                # Google Static Maps uses Mercator projection
                import math
                ...
                # Convert lat/lon bounds to Web Mercator for display
                center_x = lon_to_x_mercator(center_lon) * 6378137  # Earth radius
                center_y = lat_to_y_mercator(center_lat) * 6378137

                # Calculate pixel scale
                meters_per_pixel = 156543.03392 * math.cos(math.radians(center_lat)) / (2 ** zoom)
                img_width_m = 640 * meters_per_pixel
                img_height_m = 480 * meters_per_pixel

                extent = [
                    center_x - img_width_m/2,
                    center_x + img_width_m/2,
                    center_y - img_height_m/2,
                    center_y + img_height_m/2
                ]

                ax.imshow(map_img, extent=extent, aspect='auto', zorder=0)
                basemap_added = True
                print("[map] Google Static Maps basemap added successfully")
            else:
                print(f"[map] Google Static Maps returned status {response.status_code}")
        except Exception as e:
            print(f"[map] Could not fetch Google Static Maps: {e}")

    if not basemap_added and HAS_CONTEXTILY:
        try:
    ...
    if not basemap_added:
        print("Info: Proceeding without basemap tiles (no Google API key or contextily).")
...
if __name__ == "__main__":
    ...
    try:
        main()
        print("\n[OK] Route Map SUCCESS")
        sys.exit(0)
    except Exception as e:
        print(f"\n[FAIL] Route Map FAILED: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
