...
try:
    ...
    TEMPLATE_PATH = str(_app_paths.APP_ROOT / "Templates" / "Court Visitor Report fillable new.docx")
    STAGING_DIR = str(_app_paths.STAGING_DIR)
    DEST_BASE_FOLDER = str(_app_paths.NEW_CLIENTS_DIR)
    BACKUP_DIR = str(_app_paths.BACKUP_DIR)

    # --- Import CVR content control utilities ---
    try:
        from cvr_content_control_utils import fill_content_controls_from_row, DEFAULT_COLUMN_ALIASES
    except ImportError:
        print("WARNING: Could not import cvr_content_control_utils. Using built-in fill logic.")
        fill_content_controls_from_row = None

except Exception:
    # Fallback to hardcoded paths
    sys.path.insert(0, r"C:\GoogleSync\GuardianShip_App\Scripts")
    try:
        from cvr_content_control_utils import fill_content_controls_from_row, DEFAULT_COLUMN_ALIASES
    except ImportError:
        print("WARNING: Could not import cvr_content_control_utils. Using built-in fill logic.")
    ...
    DEST_BASE_FOLDER = r"C:\GoogleSync\GuardianShip_App\New Clients"
...
def fill_cv_info_in_doc(doc):
    """
    Fill Court Visitor information from config into CVR content controls.

    Args:
        doc: Word Document COM object

    Returns:
        Number of CV fields filled
    """
    try:
        cv_info = get_cv_info()
        filled_count = 0

        # Map content control names to CV info fields
        cv_mapping = {
            'Court Visitor Name': cv_info.get('name', ''),
            'CV Name': cv_info.get('name', ''),
        }

        # Fill content controls that match CV fields
        for cc in doc.ContentControls:
            name = (cc.Title or cc.Tag or "").strip()
            if name in cv_mapping and cv_mapping[name]:
                try:
                    cc.Range.Text = cv_mapping[name]
                    filled_count += 1
                except Exception as e:
                    print(f"  Note: Could not fill CV field '{name}': {e}")

        if filled_count > 0:
            print(f"  Filled {filled_count} Court Visitor field(s)")
...
def main():
    ...
    if not os.path.isfile(EXCEL_PATH):
        print(f"[FAIL] Organize Case Files FAILED - Excel not found: {EXCEL_PATH}")
        sys.exit(1)
    if not os.path.isfile(TEMPLATE_PATH):
        print(f"[FAIL] Organize Case Files FAILED - Template not found: {TEMPLATE_PATH}")
        sys.exit(1)
    ...
    if todo.empty:
        print("[OK] Organize Case Files - No new rows to process")
        return True
    ...
    try:
        ...
        for idx, row in todo.iterrows():
            ...
            if DRY_RUN:
                ...
                # Generate
                try:
                    # Open the template directly instead of using Add()
                ...
                if not hasattr(doc, 'ContentControls'):
                    print(f"  ERROR: Document object doesn't have ContentControls")
                    doc.Close(False)
                    continue

                # Fill content controls using utility function if available, otherwise use built-in logic
                if fill_content_controls_from_row is not None:
                    filled_count = fill_content_controls_from_row(doc, row, header_map, COLUMN_ALIASES, verbose=False)
                else:
                    # Fallback to built-in logic if utility module not available
                    filled_count = 0
                    try:
                        for cc in doc.ContentControls:
                            name = (cc.Title or cc.Tag or "").strip()
                            ...
                            if val:  # Only fill if we have a value
                                try:
                                    # Handle different content control types
                                    # Type = 3 is wdContentControlCheckBox
                                    if cc.Type == 3 and val.lower() in ['yes', 'no', 'y', 'n']:
                                        # Checkbox control
                                        cc.Checked = val.lower() in ['yes', 'y']
                                    else:
                                        # Text control (Type = 1 is wdContentControlRichText, Type = 2 is wdContentControlText)
                                        cc.Range.Text = val
                                    filled_count += 1
                                except Exception as e:
                                    print(f"  WARNING: Could not fill '{name}': {e}")
                    except Exception as e:
...
if __name__ == "__main__":
    try:
        result = main()
        if result:
            print("\n[OK] Organize Case Files SUCCESS")
            sys.exit(0)
        else:
            print("\n[FAIL] Organize Case Files FAILED - see errors above")
            sys.exit(1)
    except Exception as e:
        print(f"\n[FAIL] Organize Case Files FAILED: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
